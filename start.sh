...
if [ -d "/runpod-volume" ] && [ ! -f "$MARKER" ]; then
    ...
    mkdir -p "$VOLUME"/{diffusion_models,text_encoders,vae,clip_vision}
    ...
    wget -q --show-progress -c -O "$VOLUME/text_encoders/ltx-2-19b-embeddings_connector_dev_bf16.safetensors" \
